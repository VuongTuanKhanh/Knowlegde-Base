...
const _usfImageWidths = [295, 394, 590, 700, 900, 1200, 1500, 1800, 2000, 2400];
...
var _usfSearchResultsViewsTpl = `<div class="layout-options">
    <button type="button" title="Grid view" :class="{'active': view === 'grid'}" @click="onGridViewClick">
        <span class="icon-fallback-text">
            <span class="icon icon-grid" aria-hidden="true"><svg aria-hidden="true" focusable="false"
                    role="presentation" class="icon icon-ui-grid" viewBox="0 0 512 512">
                    <path
    ...
    <button type="button" title="List view" :class="{'active': view === 'list'}" @click="onListViewClick">
        <span class="icon-fallback-text">
            <span class="icon icon-list" aria-hidden="true"><svg aria-hidden="true" focusable="false"
                    role="presentation" class="icon icon-ui-list" viewBox="0 0 512 512">
                    <path
...
<div class="usf-sr-container" :class="{'usf-no-facets': noFacets, 'usf-empty': !loader && !hasResults, 'usf-nosearch': !showSearchBox}">
    ...
    <header class="usf-sr-config collection-header" v-if="usf.isMobile">
        ...
        <div class="sort-by">`+ _usfSearchResultsViewsTpl + `</div>
    </header>
    <header class="usf-sr-config collection-header" v-else>
        ` + _usfSearchResultsSummaryTpl + `
        <div class="sort-by"> `+ _usfSearchResultsSortByTpl + _usfSearchResultsViewsTpl + ` </div>
    </header>
    ...
    <div
        :class="(view === \'grid\' ? \'grid grid--uniform grid--view-items\' : \'list-view-items\') + \' usf-results usf-\' + view">
...
<div class="usf-sr-product grid__item small--one-half medium-up--one-third" :key="product.Id"
    ...
    <div class="grid-view-item">
        ...
        <a :href="productUrl" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave"
            ...
            <div class="usf-img-wrapper grid-view-item__image-wrapper">
                <div class="product_card__image-wrapper" :data-bgset="_usfGetImageUrls(imageUrl)">
                    <img :id="'ProductGridImage-collection-template-'+product.id" class="product_card__image lazyload"
                    :class="{'featured':(window._usfGlobalSettings['variant_rollover'] && scaledHoverImageUrl)}"
                    src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    :data-src="getImgWithSize(imageUrl,'{width}')"
                    :data-widths="'[' + _usfImageWidths.join(',') + ']'" 
                    :data-aspectratio="selectedImage.width/selectedImage.height" 
                    data-sizes="auto"
                    data-expand="600"
                    :data-fallback="getImgWithSize(scaledSelectedImageUrl,'295')"
                    :alt="product.title">

                    <img v-if="window._usfGlobalSettings['variant_rollover'] && scaledHoverImageUrl" :id="'ProductGridImage-collection-template-'+product.id" class="product_card__image alt lazyload"
                    :data-src="getImgWithSize(imageUrl,'{width}')"
                    :data-widths="'[' + _usfImageWidths.join(',') + ']'" 
                    :data-aspectratio="hoverImage.width/hoverImage.height" 
                    data-sizes="auto"
                    data-expand="600"
                    :alt="product.title">
                <!-- product image extra -->
                <usf-plugin name="searchResultsProductImageExtra" :data="pluginData"></usf-plugin>
                <span class="usf-product-label sale label--top-left" v-if="hasDiscount && !isSoldOut && usf.settings.search.showSale" v-html="loc.sale"></span>
                <span class="usf-product-label label--top-left" v-if="isSoldOut && usf.settings.search.showSoldOut" v-html="loc.soldOut"> </span>
            </div>
        ...
        <figcaption style="text-align: center;">
            ...
            <div class="product-title">
                <a :attrs="usf.plugins.invoke('getProductTitleAttrs', pluginData)" v-html="product.title"
                    class="title"></a>
                <!-- Vendor -->
                <span class="vendor"><a v-if="usf.settings.search.showVendor" class="usf-vendor grid-view-item__vendor"
                        :href="usf.platform.baseUrl + '/collections/vendors?q=' + encodeURIComponent(product.vendor)" v-html="product.vendor"></a></span>

                <!-- <span style="display:block;color: #b1b1b1;font-size: 12px;">Indoor</span> -->
            </div>
            <span class="price" :class="{'price--sold-out': isSoldOut}" v-if="!usf.plugins.lastRenderResult">
                <span class="money" v-html="displayDiscountedPrice"></span>
                <span class="money original-price" v-if="hasDiscount" v-html="displayPrice"></span>
            </span>
            <!-- price -->
...
`,
    addToCartPlugin: `
<form class="usf-add-to-cart usf-ajax-form" method="POST" enctype="multipart/form-data" :action="usf.platform.addToCartUrl" accept-charset="UTF-8"  :id="'product_form_'+variant.id">
    <input type="hidden" name="form_type" value="product">
    <input type="hidden" name="utf8" value="✓">
    <input type="hidden" name="quantity" value="1">
    <input type="hidden" name="id" :value="variant.id">
    <button type="submit" name="add" :class="{'usf-visible': args.isHover}" class="usf-add-to-cart-btn" data-addtocart><span data-addtocarttext v-html="loc.addToCart"></span></button>
</form>
`,
    // Preview Plugin
    previewPlugin: `
